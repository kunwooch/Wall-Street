...
def main():
    ...
    f1 = scipy.io.loadmat('/home/kwcho/Desktop/controller/DACData/mb_soft_ho1.mat')
    f2 = scipy.io.loadmat('/home/kwcho/Desktop/controller/DACData/mb_soft_ho2.mat')

    f_single = scipy.io.loadmat('/home/kwcho/Desktop/controller/DACData/tra_steering_S11_S21_1deg_26ghz.mat')
    ...
    ue_list_single = f_single['ue_list']
    um_list_single = f_single['um_list']
    ...
    while userInput != "exit":
        ...
        [userInputBytes, sender] = inSocket.recvfrom(10)
        ...
        if userInput == "exit":
            ...
            try:
                inputs = userInput.split("_")

                input_y = int(inputs[0])
                input_x = int(inputs[1])
                input_fn = int(inputs[2])

            except ValueError:
                print("Not an integer!")
                continue
            else:
                print("Input y: %2d, input x: %2d, file: %1d" % (input_y, input_x, input_fn))
                start = time.time()

                y_list = f1['loc'][:, 0].flatten()
                x_list = f1['loc'][:, 1].flatten()
 
                y_idx = np.where(y_list == input_y)[0]
                x_idx = np.where(x_list == input_x)[0]

                common_idx = list(set(y_idx) & set(x_idx))[0]

                if input_fn == 0:
                    ue_list = ue_list1
                    um_list = um_list1
                    tx1_list = f1['Tx_angle1'].flatten()
                    tx2_list = f1['Tx_angle2'].flatten()
                    print("common_idx: %2d, tx1 angle: %2d, tx2 angle: %2d" % (common_idx, tx1_list[common_idx], tx2_list[common_idx]))
                    ue = ue_list[common_idx, ::-2].round(2)
                    um = um_list[common_idx, ::-2].round(2)
                elif input_fn == 1:
                    ue_list = ue_list2
                    um_list = um_list2
                    tx1_list = f2['Tx_angle1'].flatten()
                    tx2_list = f2['Tx_angle2'].flatten()
                    print("common_idx: %2d, tx1 angle: %2d, tx2 angle: %2d" % (common_idx, tx1_list[common_idx], tx2_list[common_idx]))
                    ue = ue_list[common_idx, ::-2].round(2)
                    um = um_list[common_idx, ::-2].round(2)
                elif input_fn == 2:
                    ue_list = ue_list_single
                    um_list = um_list_single
                    tx1_list = f1['Tx_angle1'].flatten()
                    print("common_idx: %2d, tx1 angle: %2d" % (common_idx, tx1_list[common_idx]))
                    Rx_idx = np.where(f_single['Rx_angle'].flatten() == tx1_list[common_idx])[0][0]
                    Tx_idx = np.where(f_single['Tx_angle'].flatten() == 0)[0][0]
                    ...
                    um_list = um_list_single
                    tx2_list = f1['Tx_angle2'].flatten()
                    print("common_idx: %2d, tx2 angle: %2d" % (common_idx, tx2_list[common_idx]))
                    Rx_idx = np.where(f_single['Rx_angle'].flatten() == tx2_list[common_idx])[0][0]
                    Tx_idx = np.where(f_single['Tx_angle'].flatten() == 0)[0][0]
                    ue = ue_list[Tx_idx, Rx_idx, ::-2].round(2)
                    um = um_list[Tx_idx, Rx_idx, ::-2].round(2)
