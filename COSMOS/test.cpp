...
void find_opt_beams_surface(paam* tx, mmwall_ctl* wall, ofdm_tx* pkt_tx, ofdm_rx* pkt_rx, size_t* tx_indices, uint32_t tx_num, int tx_theta_list[], int tx_angle_step, int tx_angle_range, int wall_angle_step, float* max_rsrp)
{
	...
	if(tx_num > 1){
        ...
        int tx_phi = 0;
        int rx_phi = 0;
        int opt_tx_theta[tx_num], opt_wall_theta[tx_num];
        float rsrp = -100000;
        uint32_t rx_pkts = 0;
        float per = 0;
	...
	int tx_theta_final = 0;
        ...
        for(int tx_theta = -tx_angle_range; tx_theta <= tx_angle_range; tx_theta += tx_angle_step){
	     ...
	     // std::cout << "tx_theta: " << tx_theta;
	     for(size_t i = 0; i < tx_num; i++){
	        // std::cout << "		||	tx_theta_" << i  << ": " << tx_theta_list[local_tx_indices[i]] + tx_theta;
		...
		for(size_t i = 0; i < tx_num; i++){
		    tx_theta_final = tx_theta_list[local_tx_indices[i]] + tx_theta;
                    g_logfile[local_tx_indices[i]] << "  " << std::setw(4) << tx_theta_final << "   " << "  " << std::setw(4) << wall_theta << "   ";
...
int main(int argc, char* argv[])
{
    ...
    if(argc < 4){
	std::cout << "Please enter tx_angle_range, tx_angle_step, and wall_angle_step" << std::endl;
	return 0;
    }else{
	tx_angle_range = std::stoi(std::string(argv[1]));
	tx_angle_step_size = std::stoi(std::string(argv[2]));    
	wall_angle_step_size = std::stoi(std::string(argv[3]));    
    }
	...
	if(more_data == 'y'){
	    ...
	    size_t tx_indices[2] = {0,1};
            ...
            find_opt_beams_surface(paam_tx, &rpi1, pkt_tx, pkt_rx, tx_indices, 2, tx_theta_list, tx_angle_step_size, tx_angle_range, wall_angle_step_size, max_rsrp);
	    ...
	    if (NUM_GNB == 3){
                find_opt_beams_surface(paam_tx, &rpi1, pkt_tx, pkt_rx, tx_indices, 1, tx_theta_list, tx_angle_step_size, tx_angle_range, wall_angle_step_size, max_rsrp); 
	    }
