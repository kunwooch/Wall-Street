...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    uint32_t rx_port = 1234;
    const char* rx_ip = "10.37.1.1";
	...
	for(uint32_t i = 0; i < num_pkts; i++){
	    if (type == "double")
            ...
            std::this_thread::sleep_for(std::chrono::microseconds(int64_t(recv_data[1]*1000)));
        }
	std::cout << "Sent " << num_pkts  << " packets with a gap of " << recv_data[1] << "ms" << std::endl;
