...
{   
    ...
    if(argc < 5){
	std::cout << "Please enter the UE mob index[1-2], gNb index[0-2], second per meter, and the path file:" ;
    }else{
 	ue_index = std::stoi(std::string(argv[1]));
	gnb_index = std::stoi(std::string(argv[2])); 
	speed = std::stod(std::string(argv[3]));
	path_file = std::string(argv[4]);
    }    

    std::string angle_file = "lookup/baseline_enb" + std::string(argv[2])  + "_ue" + std::string(argv[1]) + ".txt";
    ...
    */
    ...
    g_logfile << "Time       Loc         gNb Theta   UE Theta    RSRP   NumPkts  PER" << std::endl; 
    g_logfile << "***************************************************************************" << std::endl;
    ...
    // indoor UE moves at 1m/12s = 0.083m/s
    // Each RSRP measurement for 2 gNbs takes 50*0.6 + 100 ms = 0.13 s --> 0.185 s (measured without surface) and 0.25 s (measured with surface & 100 ms sleep) and 0.145 s (measured with surface)
    // Lets say we take 6 measurements per sec. 72 measurements per meter.
	...
	//for(size_t i =0; i < num_data + 100; i++){ // collect 10 extra 
	while(1){
	    ...
	    g_logfile << std::setw(7) << double(elapsed_seconds.count()) << "    " << std::setw(10) << loc << "    "; 		
	    ...
	    g_logfile << std::setw(7)<< double(enb_theta) << "    " << std::setw(7)<< double(ue_theta) << "    ";

	    float rsrp = get_rsrp(&pkt_tx, pkt_rx);
	    g_rsrpfile << rsrp << std::endl;
       }
    }

    std::cout << "Terminating the program..." << loc << std::endl;
