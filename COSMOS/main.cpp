...
void find_opt_beam(paam* tx, paam*rx, ofdm_txrx* pkt_ctrl, uint32_t tx_num_pkts, double tx_pkt_interval, uint32_t tx_angle_step, uint32_t rx_angle_step, std::string filename)
{
	// open file to write rsrp and per while sweeping tx and rx beams
	std::ofstream rsrpfile;
	rsrpfile.open(filename);
	rsrpfile << "TX Theta RX Theta    RSRP      RX_PKTS    PER" << std::endl;
        rsrpfile << "===============================================" << std::endl;	
	...
	uint32_t opt_tx_theta = 0;
	uint32_t opt_rx_theta = 0;
	...
	for(int tx_theta = -60; tx_theta <= 60; tx_theta+=tx_angle_step){
		...
		//report and record metrics
	 	pkt_ctrl->report_metrics(&avg_rsrp, &rx_pkts, &per, tx_num_pkts);
                rsrpfile << "  " << std::setw(4) << tx_theta << "   " << "  " << std::setw(4) << rx_theta << "   " << std::fixed << std::setprecision(6) << std::setw(10)<< avg_rsrp << "  " << std::setw(5) << rx_pkts << "  "<< std::setprecision(7) << per << std::endl;	
	        if(max_rsrp < avg_rsrp){
		    max_rsrp = avg_rsrp;
	...
	rsrpfile << std::endl << "Opt TX theta " << opt_tx_theta << " Opt RX theta " << opt_rx_theta << " RSRP :" << max_rsrp << std::endl;
}

/*void find_opt_beams_2(paam* tx1, paam* tx2, paam*rx, ofdm_txrx* pkt_ctrl, uint32_t tx_num_pkts, double tx_pkt_interval, uint32_t tx_angle_step, uint32_t rx_angle_step, std::string filename)
{
        // open file to write rsrp and per while sweeping tx and rx beams
        std::ofstream rsrpfile;
        rsrpfile.open(filename);
        ...
        for(int tx_theta = -60; tx_theta <= 60; tx_theta+=tx_angle_step){
            ...
            for(int rx_theta = -60; rx_theta <= 60; rx_theta+=rx_angle_step){
                ...
                //report and record metrics
                pkt_ctrl->report_metrics(&avg_rsrp, &rx_pkts, &per, tx_num_pkts);
                rsrpfile << "  " << std::setw(4) << tx_theta << "   " << "  " << std::setw(4) << rx_theta << "   " << std::fixed << std::setprecision(8) << avg_rsrp << "  " << std::setw(5) << rx_pkts << "  "<< std::setprecision(7) << per << std::endl;
            }
        }
}*/
...
int main()
{
    ...
    paam rfdev4_in2("rfdev4-in2.sb1.cosmos-lab.org");
    rfdev_mob4_1.enable("all", 16, "tx", 'h');
    //rfdev_mob4_2.enable("all", 16, "rx", 'h');
    rfdev4_in2.enable("all", 16, "rx", 'h');
