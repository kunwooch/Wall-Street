...
{   
    ...
    if(argc < 5){
	std::cout << "Please enter the UE mob index[1-2], gNb index[0-2], second per meter, path file, and the offset:" ;
    }else{
	...
	offset = std::stoi(std::string(argv[5]));
    }    

    std::string angle_file = "lookup/surface_enb" + std::string(argv[2])  + "_ue" + std::string(argv[1]) + ".txt";
    ...
    */
	...
	while(1){
	    ...
	    if(wall_theta + offset > 70)
	    {
		wall_theta = 70;
	    }
	    else{
	        wall_theta = wall_theta + offset;
	    }
