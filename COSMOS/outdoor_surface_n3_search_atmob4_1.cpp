...
void find_opt_beams_surface(paam* tx, paam* rx, mmwall_ctl* wall, ofdm_tx* pkt_tx, ofdm_rx* pkt_rx, size_t* tx_indices, uint32_t tx_num, int tx_theta_list[], int tx_angle_step, int tx_angle_range, int rx_angle_step, int rx_angle_range, int wall_angle_step, float* max_rsrp)
{
	...
	int tx_theta_final = 0;
        ...
        for(size_t i = 0; i < tx_num; i++){
            g_logfile[local_tx_indices[i]] << "TX Theta	 Rx Theta  WallRXTheta    RSRP      RX_PKTS     PER" << std::endl;
        ...
        //Sweep tx theta
        for(int tx_theta = -tx_angle_range; tx_theta <= tx_angle_range; tx_theta += tx_angle_step){
	    	
	     for(size_t i = 0; i < tx_num; i++){
            	(tx + local_tx_indices[i])->steer(tx_theta_list[local_tx_indices[i]] + tx_theta, tx_phi);
             }
		...
		for(int wall_theta = -70; wall_theta <= 70; wall_theta+=wall_angle_step){
			...
			for(size_t i = 0; i < tx_num; i++){
			    tx_theta_final = tx_theta_list[local_tx_indices[i]] + tx_theta;
			    g_logfile[local_tx_indices[i]] << "  " << std::setw(4) << tx_theta_final << "   " << "  " << std::setw(4) << rx_theta << "   " << "  " << std::setw(4) << wall_theta << "   ";
        ...
        for(size_t i=0; i < tx_num; i++){
            g_logfile[local_tx_indices[i]] << std::endl << "Opt TX theta " << opt_tx_theta[i] << " Opt Rx theta" << opt_rx_theta[i] << " Opt WallRX theta " << opt_wall_theta[i] << " RSRP :" << *(max_rsrp + local_tx_indices[i]) << std::endl;
        }

        for(size_t i=0; i < tx_num; i++){
            g_rsrpfile[local_tx_indices[i]] << std::setw(6) << opt_tx_theta[i] << "        " <<  opt_rx_theta[i] << "        " << std::setw(6) << opt_wall_theta[i] << "        " << *(max_rsrp + local_tx_indices[i]) << std::endl;
        }
}
...
int main(int argc, char* argv[])
{
    ...
    int tx_angle_range = 20;    
    int ue_angle_range = 20;
    if(argc < 6){
	std::cout << "Please enter tx_angle_range, tx_angle_step, ue_angle_range, ue_angle_step_size, and wall_angle_step" << std::endl;
	return 0;
    }else{
	tx_angle_range = std::stoi(std::string(argv[1]));
	tx_angle_step_size = std::stoi(std::string(argv[2]));  
        ue_angle_range = std::stoi(std::string(argv[3]));
	ue_angle_step_size = std::stoi(std::string(argv[4]));	
	wall_angle_step_size = std::stoi(std::string(argv[5]));    
    }
    ...
    for(size_t i = 0; i < NUM_GNB; i++){
        ...
        g_rsrpfile[i] << "RX location Information         Opt TX Theta     Opt RX Theta    Opt Wall Theta     Max RSRP" << std::endl;
        g_rsrpfile[i] << "********************************************************************************************" << std::endl;
        }
	...
	if(more_data == 'y'){
		...
		int* loc_angle;
            	...
            	if(loc_angle_pair != g_angle_map.end()){
                	...
                	std::cout << "Can't find location data for " << loc << std::endl;
                	break;
            	}
		...
		if(tx_theta + tx_angle_range > 60){	
                     tx_theta = 60 - tx_angle_range;
		     std::cout << "This angle is out of the range. The angle is set to " << tx_theta << std::endl;
		}
		if(tx_theta - tx_angle_range < -60){
		     tx_theta = -60 + tx_angle_range;
		     std::cout << "This angle is out of the range. The angle is set to " << tx_theta << std::endl;
		...
		wall_theta_list[gNb_index] = wall_theta;
            }
	    ...
	    size_t tx_indices[2] = {0,1};
            ...
            find_opt_beams_surface(paam_tx, &paam_rx, &rpi1, pkt_tx, pkt_rx, tx_indices, 2, tx_theta_list, tx_angle_step_size, tx_angle_range, ue_angle_step_size, ue_angle_range, wall_angle_step_size, max_rsrp);
	    ...
	    if (NUM_GNB == 3){
                find_opt_beams_surface(paam_tx, &paam_rx, &rpi1, pkt_tx, pkt_rx, tx_indices, 1, tx_theta_list, tx_angle_step_size, tx_angle_range, ue_angle_step_size, ue_angle_range, wall_angle_step_size, max_rsrp); 
	    }
