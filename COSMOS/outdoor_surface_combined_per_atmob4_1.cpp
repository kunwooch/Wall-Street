...
void get_combined_rsrp(ofdm_tx* pkt_tx, ofdm_rx* pkt_rx)
{
    ...
    uint32_t tx_pkts = (pkt_tx)->get_num_pkts();
    //uint32_t tx_pkts1 = (pkt_tx)->get_num_pkts();
    (pkt_rx)->report_metrics(&rsrp, &rx_pkts1, &per, tx_pkts, tx1_pkt_num_set);
    ...
    (pkt_rx + 1)->report_metrics(&rsrp, &rx_pkts2, &per, tx_pkts, tx2_pkt_num_set);
    ...
    g_logfile << std::setw(7)<< com_pkt_num_set.size() << "       " << (tx_pkts - com_pkt_num_set.size())*100/tx_pkts << std::endl;	
}
...
{   
    ...
    //Parse angle files
    std::string angle_file1 = "lookup/surface_enb" + std::to_string(gnb1_index) + "_ue" + std::to_string(ue_index) + ".txt";
    std::string angle_file2 = "lookup/surface_enb" + std::to_string(gnb2_index) + "_ue" + std::to_string(ue_index) + ".txt";

    std::cout << "gNB1: " << std::to_string(gnb1_index) << "  gNB2: " << std::to_string(gnb2_index) << "  UE: " << std::to_string(ue_index) << std::endl;
    ...
    g_logfile << "Time        Loc   TX1Theta  Wall1Theta  TX2Theta  Wall2Theta  RSRP1    RX_PKTS1      PER1      RSRP2    RX_PKTS2    PER2  COM_RX_PKTS  COM_PER" << std::endl;
    g_logfile << "==================================================================================================================================================" << std::endl;
	...
	while(1){
		...
		g_logfile << std::setw(7)<< int(wall_theta) << "    ";
    	    }
