...
int main()
{
    ...
    //Open Log file
    g_logfile.open("doa_log.txt");	

    // Create paam objects for PAAMs 	
    paam paam_tx("rfdev-mob4-3.sb1.cosmos-lab.org");
    paam paam_rx("rfdev4-in1.sb1.cosmos-lab.org");
    // Enable the PAAMs - in tx mode, in rx mode
    paam_rx.enable("all", 16, "rx", 'v', 0);
    paam_tx.enable("all", 16, "tx", 'v', -250000 );
    ...
    float rsrp;
    for(int rx_angle = 45; rx_angle >= 0; rx_angle -= 5)
    {
        xytable_rx.move(xytable_pos(10,10, rx_angle));
	std::this_thread::sleep_for(std::chrono::milliseconds(3000));
	std::cout << std::endl << "XYtable angle : " << rx_angle << std::endl;
	g_logfile << "XYtable angle : " << rx_angle << std::endl;
        find_opt_rx_beam(&paam_tx, &paam_rx, &pkt_tx, &pkt_rx, 1);
        rsrp = get_rsrp(&pkt_tx, &pkt_rx);
